...
public class BoxEntity(
    ...
    internal fun sendCreatePacket(player: Player) {
        ...
        this.teleportToWithTicket(playerLocation.getX(), playerLocation.getY(), playerLocation.getZ())

        // println("sendUpdateCrawlPacket $loc")

        // send packets to create fake shulker box
        val packetSpawn = ClientboundTeleportEntityPacket(this)

        // entity metadata
        // https://wiki.vg/Entity_metadata#Shulker
        val packetEntityData = ClientboundSetEntityDataPacket(this.getId(), this.getEntityData().getNonDefaultValues())
    ...
    internal fun sendPeekMetadataPacket(player: Player) {
        val packet = ClientboundSetEntityDataPacket(this.getId(), this.getEntityData().getNonDefaultValues())
        (player as CraftPlayer).handle.connection.send(packet)
    }
}